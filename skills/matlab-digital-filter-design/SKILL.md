...
- **Do not guess key requirements.** If *Mode* (streaming vs offline) or *Phase requirement* is not stated, **ask**.  
  You may analyze the signal first (spectrum, peaks, bandwidth), but you must not silently commit to `filtfilt()` or a linearâ€‘phase design without the userâ€™s intent.
...
- **IIR stability:** prefer **SOS/CTF** forms (avoid highâ€‘order `[b,a]` polynomials).
...
- If errors occur, edit the file and rerun â€?don't put all code inline in tool calls
...
If any item is missing â†?**ask**.

### Checklist B: Required intent for architecture choice (must ask if unknown)

- [ ] **Mode**: streaming (causal) | offline (batch)
- [ ] **Phase**: zeroâ€‘phase | linearâ€‘phase | donâ€™tâ€‘care
...
If Mode or Phase is unknown: ask **1â€?** clarifying questions and stop.  
Do **not** assume â€œofflineâ€?or â€œzeroâ€‘phaseâ€?
...
- `trans_pct > 5%` â†?singleâ€‘stage FIR or IIR is usually fine
- `2% â‰?trans_pct â‰?5%` â†?singleâ€‘stage is possible; mention efficient alternatives if cost/latency matters
- `trans_pct < 2%` â†?**STOP and do a narrowâ€‘transition comparison**
  Open `knowledge/cards/efficient-filtering.md`.

**Important:** for `trans_pct < 2%`, do **not** blindly show all four families.  
Select and present only the **viable** candidates given Mode + Phase, and explicitly mark excluded families (with a oneâ€‘line reason).
...
1. **Feasibility / order sanity check**
   ...
   - Optional (especially for narrow transitions): use `kaiserord` / `firpmord` to estimate FIR length for planning (not as â€œthe truthâ€?.
...
2. **Design candidates**
   ...
   - Offline zeroâ€‘phase: `filtfilt()` is allowed, but you must state:
     - forwardâ€‘backward filtering **squares magnitude** (â‰?doubles dB attenuation) and effectively doubles order.
...
4. **Verify with numbers (not just plots)**
   - Worstâ€‘case passband ripple and stopband attenuation vs spec.
...
Thatâ€™s the whole job: make the workflow predictable, and make the assumptions impossible to miss.
